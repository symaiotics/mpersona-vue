    ...
    <div class="flex dark:bg-gray-800">
        ...
        <div class="w-1/2 p-4 dark:bg-gray-900" v-if="selectedPersona">
            <form @submit.prevent>
                ...
                <div class="mb-4" v-if="selectedPersona.isOwner || selectedPersona.isEditor">
                    ...
                    <div class="flex space-x-4">
                        <button @click="createLink('viewerLink')"
                            ...
                            Viewer Link</button>
                        <button v-if="selectedPersona.viewerLink" @click="copyToClipboard(selectedPersona.viewerLink)"
                ...
                <div class="mb-4" v-if="selectedPersona.isOwner || selectedPersona.isEditor">
                    ...
                    <div class="flex space-x-4">
                        <button @click="createLink('editorLink')"
                            class="px-4 py-2 bg-blue-500 text-white dark:bg-blue-700">Create
                            Editor Link</button>
                        <button v-if="selectedPersona.editorLink" @click="copyToClipboard(selectedPersona.editorLink)"
                            class="px-4 py-2 bg-gray-500 text-white dark:bg-gray-700">Copy Link</button>
                    </div>
                </div>
                ...
                <div class="flex space-x-4">
                    <button v-if="selectedPersona.isOwner || selectedPersona.isEditor" @click="updatePersona"
                        class="px-4 py-2 bg-green-500 text-white dark:bg-green-700">Update
                        Persona</button>
                    <button v-if="selectedPersona.isOwner" @click="deletePersona"
                        class="px-4 py-2 bg-yellow-500 text-white dark:bg-yellow-700">Delete
                        Persona</button>
                </div>
            </form>
        </div>
...
function createLink(linkType) {
    if (selectedPersona.value.isEditor) {
        ...
        addLink(selectedPersona.value._id, words, linkType).then((response) => {
